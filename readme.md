...
enter an item identifier and requirements such as `efficiency:4,unbreaking:3`, then find a route. namespaced identifiers work too. requirements are minimum levels, so an extra compatible enchantment is allowed.
...
normal walking no longer clears a recovered seed. camera or position changes still cancel an active drop sequence because its item-spawn matching depends on a fixed origin. damage, sprinting, swimming, fire, item use, block breaking, equipment changes, experience pickups, mending, advancements, respawns, dimension changes, untracked inventory drops, creative inventory changes, and relevant server player events conservatively invalidate player rng tracking. tracked drops advance rng by four calls only when doctrine issues them, and server spawn packets confirm progress. a mismatch or confirmation timeout clears the recovery state.
